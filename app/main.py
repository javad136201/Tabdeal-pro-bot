...
async def collector():
    while True:
        try:
            ...
            new_cs=candles(raw)
            if new_cs:
                # قبلاً هر بار کل تاریخچه با فقط همین ۱۰۰۰ ترید اخیر
                # جایگزین می‌شد (state["candles"]=cs) و کندل‌های قدیمی‌تر
                # از دست می‌رفتند. حالا کندل‌های جدید با تاریخچه‌ی موجود
                # merge می‌شوند (کندلِ در حالِ شکل‌گیریِ دقیقه‌ی جاری آپدیت
                # می‌شود، بقیه اضافه می‌شوند) و فقط سقف MAX_CANDLES نگه داشته می‌شود.
                merged={c["time"]:c for c in state["candles"]}
                for c in new_cs:
                    merged[c["time"]]=c
                cs=sorted(merged.values(), key=lambda x:x["time"])
