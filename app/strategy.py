def analyze(c):
    if c is None or len(c)<55: return {"signal":"HOLD","confidence":0,"price":None,"reason":"داده کافی نیست.","candles":0 if c is None else len(c)}
    ...
    rs=g/l.replace(0,None); x["rsi"]=100-100/(1+rs); x["vma"]=x.volume.rolling(20).mean(); z=x.iloc[-1]
    p=float(z.close); e20=float(z.ema20); e50=float(z.ema50); r=float(z.rsi) if z.rsi==z.rsi else 50; vr=float(z.volume/z.vma) if z.vma and z.vma==z.vma else 1
    score=0; reasons=[]
