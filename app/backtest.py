...
def run(candles, balance=1000, trade_usd=50, fee_pct=0.1, tp_pct=1.0, sl_pct=0.6, min_confidence=65):
    ...
    for i in range(55,len(candles)):
        ...
        if pos:
            if p>=pos["tp"] or p<=pos["sl"]:
                ...
                pos=None
        if not pos and a.get("signal")=="BUY" and a.get("confidence",0)>=min_confidence and cash>=trade_usd:
