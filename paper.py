...
class Paper:
    ...
    def reset(self): self.usd=self.start;self.qty=0.;self.entry=0.;self.last=0.;self.sl=self.tp=0.;self.high=0.;self.realized=0.;self.fees=0.;self.trades=[];self.enabled=True;self.last_trade=0.;self.today_trades=0;self.day_start=self.start;self.day=datetime.date.today()
    def _limits_ok(self):
        if datetime.date.today()!=self.day:self.day=datetime.date.today();self.today_trades=0;self.day_start=self.equity()
        if self.today_trades>=int(self.cfg.max_trades_per_day):return False
        if self.equity() <= self.start*(1-float(self.cfg.max_daily_loss_pct)/100):return False
        return True
    def equity(self):return self.usd+self.qty*self.last
    def buy(self,p,sl,tp):
        if self.qty or not self.enabled or time.time()-self.last_trade<float(self.cfg.cooldown_seconds) or not self._limits_ok():return False
        spend=min(float(self.order),self.usd);f=spend*self.fee;q=(spend-f)/p
        if q<=0:return False
    ...
    def sell(self,p,reason):
        ...
        q=self.qty;gross=q*p;f=gross*self.fee;net=gross-f;pnl=net-q*self.entry;self.usd+=net;self.qty=0;self.entry=0;self.high=0;self.sl=self.tp=0;self.fees+=f;self.realized+=pnl;self.last_trade=time.time();self.today_trades+=1
